...
//    }
    ...
    void findAllSimpleCycles(int start, int current, int depth, int sub_graph_index) {
        ...
        for (auto &adjacent : graph[current]) {
            if (adjacent == start) {
                ...
                if (depth < 5 && scc_result[sub_graph_index].count(adjacent) != 0) {
                    findAllSimpleCycles(start, adjacent, depth + 1, sub_graph_index);
                } else if (depth == 5) {
